...
E.g.:
    ...
    def setPred(self, start=None, to=None, pred=None):
        ...
        """
        if False:
            self.vertList[start].connectedTo[to][1] = []
        else:
            if pred:
                if pred != start:
                    try:
                        self.vertList[start].connectedTo[to][1] = [pred]+self.vertList[start].connectedTo[pred][1]
                    except IndexError:
    ...
    def clean_up(self, start):
        for x in self.vertList:
            for y in self.vertList[x].getConnections():
                try:
                    if not len(self.vertList[x].connectedTo[y][1]):
                        self.vertList[x].connectedTo[y].pop(-1)
                except IndexError:
                    pass
        del self.vertList[start].connectedTo[start]
    ...
    def dijkstra(self, start):
        ...
        self.populatePaths()
        visited = {start}
        self.setPred(start,start)
        first = True
        while first or not queue.isEmpty():
            queue = HeapPriorityQueue()
            for to in self.vertList[start].getConnections():
                if first:
                    self.setPred(start,to, start)
                if to not in visited:
                    queue.heapInsert([(self.getDistance(start, to), to)])
            first = False
            if queue.isEmpty():
                break
            dist, node = queue.popMin()
            visited.add(node)
            for to in self.vertList[node].connectedTo:
                if dist + self.getDistance(node, to) < self.getDistance(start, to):
                    self.setDistance(start, to, dist + self.getDistance(node, to))
                    self.setPred(start, to, node)
        self.clean_up(start)
    ...
    def betweenness_func(self):
        ...
        """
        dictionary_of_betweenness = {node:0 for node in self.vertList}
        for node in dictionary_of_betweenness:
            self.dijkstra(node)
            for to in self.getVertex(node).getConnections():
                try:
                    for x in self.getVertex(node).connectedTo[to][1]:
                        dictionary_of_betweenness[x] += 1
                except IndexError:
                    pass
        max_bc_node_id = sorted([(dictionary_of_betweenness[node],node) for node in dictionary_of_betweenness], reverse=True)[0][1]
        return max_bc_node_id, {key:dictionary_of_betweenness[key] for key in dictionary_of_betweenness} 
