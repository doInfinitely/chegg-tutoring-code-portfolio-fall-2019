...
This copy of Graph is intentionally broken.  The problem involves debuging and fixing it.
 
Debuging and fixing this file is a 2 step problem:
First we have to make sure it compiles.
The second steps involves fiing the algothrithm implementations.

Attached to this email is a .cpp tester file
The output that the .cpp file produces must match the correspending cout statement written in that file (for e.g. cout << "Path A to F is 8: " << myGraph.BestDistance("A", "F", &path) << endl;
must produce "Path A to F is 8: 8" 
 ****************************************************************************************************************************************/
...
class Graph {
	...
	bool Hasloopinternal()
	{
		...
		for( auto mapIter = _mainData.begin(); mapIter != _mainData.end(); ++mapIter ) {
			if( (*mapIter).second->_hasLoopIns != 0 )
				return true;
		}
	...
	bool HasLoopExternal()
	{
		...
		std::map<std::string, std::string> hasLoopIns;
	...
	bool HasLoopCopy()
	{
		...
		for( std::unordered_map<std::string, std::string>::iterator mapIter = copyGraph._mainData.begin(); mapIter != copyGraph._mainData.end(); ++mapIter ) {
...
public:
	...
	void Connect(std::string from, std::string to, int weight) {
		...
		_mainData[to]->_outs.push_back(newEdge);
		_mainData[from]->_ins.push_back(newEdge);
	}
	...
	int BestDistance(std::string from, std::string to, std::list<std::string> *path) {
		std::map<std::string, std::string> currentDistance;
		...
		do {
			...
			for( auto mapIter = _mainData.begin(); mapIter != _mainData.end(); ++mapIter )
			{
				if( currentDistance[(*mapIter).first] < bestDistance ) {
					found = true;
			...
			if( found ) {
				...
				for( auto outIter : current->_outs ) {
					int potential = currentDistance[current->_data];
